...
| 2026-04-21 08:01 CEST | S8 coordinated disabled-mode window with Archidoc | **PASS.** Archidoc flipped `OUVRO_SNAGS_ENABLED=false` on staging and DM'd "snags disabled, go" at 08:01. Single `npx tsx smoke-snags.ts --include-disabled` fired immediately; S8 returned `status: 503` with `code: "FEATURE_DISABLED"` and `error: "Snag intake is not enabled for this tenant."` — exact match to the documented contract. S1–S6 in the same run also returned 503 (expected: every probe during the disabled window naturally hits the same gate); S7 (direct-staging bad bearer) still returned 401 since auth is checked outside the feature gate. Window closed at ~08:06 with Archidoc's "snags re-enabled" DM. Run log: `.local/smoke-logs/run-2026-04-21T06-07-53-597Z.json`. Snag intake validation against staging is now **fully closed out** (S1–S8 all observed green at least once). |
...
| 2026-04-21 | Task #17 — re-run staging snag smoke after Archidoc deployed the intake route at the same path (`POST /api/ouvro/snags`) | **PASS 8/8** (S1–S7 + S8 skipped per coordinated disabled-mode protocol). Run log: `.local/smoke-logs/run-2026-04-21T05-44-58-883Z.json`. Two upstream contract observations were caught and patched on our side: (1) Archidoc's deployed intake **now requires `media[i].fileName`** (`media.0.fileName: Invalid input: expected string, received undefined` on first re-run) — added `fileName?: string` to `SnagMediaItem` (`client/lib/archidoc-types.ts`) and `SnagMediaInput` (`server/routes/snags.ts`), promoted it to a required field in `validateMediaItem`, and threaded it through the smoke runner using the file name returned by `POST /api/uploads/request-url`. (2) Archidoc currently **accepts whitespace-only `title` and returns 201** instead of 400 `VALIDATION_FAILED` — hardened the BFF guard to reject whitespace titles defensively and tagged every BFF-side 400 with `code: "VALIDATION_FAILED"` so clients can branch on a stable error vocabulary. Audit-log echo (Archidoc deep links): S1 `…?tab=defauts&item=13`, S3 `…?tab=reserves&item=15` — confirms tab selectors agree with the contract. Existing `server/routes/__tests__/snags.test.ts` updated to include `fileName` in `VALID_BODY`; full unit suite still green (12/12). S8 (`OUVRO_SNAGS_ENABLED=false → 503 FEATURE_DISABLED`) needs a coordinated 5-minute disabled-mode window with Archidoc; re-run with `npx tsx smoke-snags.ts --include-disabled` once the window is open. |
